...
public static class RegistrationRepositoryQueries
{
    public const string GetavailableTimes = @"WITH all_hours as(
                                                ...
                                                SELECT * FROM generate_series(
                                                    @date + (SELECT mt.start_time FROM mytime mt),
                                                    @date + (SELECT mt.end_time FROM mytime mt), '1 hour') as hour
                                                )
                                                SELECT hour
                                                FROM all_hours ah
                                                WHERE ah.hour not in (select r.start_time from registrations r WHERE r.employee_id = @employeeId)";
}